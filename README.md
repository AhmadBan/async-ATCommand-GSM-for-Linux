...
just Add the Address of your serial port and period of polling in seconds and thats it !!!
...
executing a command can be either synchronously and asynchronously.

### **ASynchronous**

To execute a command Asynchronously first instantiate an instance of that command from heap then initialize it with its constructors then send it to the program Command Queue.
...
For some commands that has additional parameter you should initialize the parameter outside the constructor. for example to send an SMS

```C
/*Command structure for sending a SMS
 *Notic that Cmd_t must be the first element of the Struct so that this struct is a child of Cmd_t
 */
...
Finally to add you custom callback function to executes in case of an error or a success just initialize "fpCallBackOnSuccess" and "fpCallBackOnError" with your own function. for example
...
Notice that by default if your a command executes successfully it is automatically removed from Command Queue and then it is freed from heap and otherwise the command stays in Command Queue and executes again.

### **Synchronous**

Executing a command synchronously is similar to above but instead of executing command in another thread you should call "fpProc" function of that command in your program and wait for it to finish. However you should initialize the port parameter while you are instantiating the command and also you are responsible to free your allocated memory yourself.
...
Notice you can get an instance of communicating port from GSM_t struct that its refrence is returned from "gsmSetup" function
...
All commands follow a struct interface "Cmd_t". This interface contains all basic properties and methods that is necessary for a specific command. For example it includes some properties that keeps command charactristics such as id, and delays that is needed for command in each stage as well as methods that are used mainly to execute the command and get the result as a function pointer such as fpRequest, fpResponse and fpProc
...
**command[50]**: it is a string that contains command string for that command for example for reading SMS it is "AT+CMGL="

**finishParam[20]**:it is a string that contains parameter for that string to finish command for example for reading SMS it is "\"ALL\"\r".

**fpRequest**: this is a function that is responsible to send request command.

**fpResponse**: this is a function that is responsible to process response from command.

**fpProc**: this is a function that is responsible to process request and response command and return the result to callbacks.
...
# Add a new command to library

AT library has a lot of command and I design architecture so it can be really easy to add a new command to it.

If you want to add a simple two-part command ( commands that send a command and receives ok or an error as responses) to it just create a .C file in src/commands folder and make a constructor file to initialize Cmd_t structure for that command such as command string and delays needed. for example
...
in case of two-part command using this constructor is sufficient however, in case of more complex command you would better override your own fpProc,fpRequest and fpResponses. Morever if you need more properties than in the Cmd_t struct you need to define your custom struct as explained above to be derieved from Cmd_t. for example in case of reading All SMS I need an extra parameter to maintain SMS information so making the struct **ReadSMS_t** like below.
...
I need to implement two custom process and response method other than default function in basicCommand.c file so I override them as below.
...
```
and then I used them in constructor to initialize function pointer for that command.
